...
# SQLAlchemy setup
engine = create_engine('sqlite:///drone_data_MAIN.db.sqlite3')  # Create an SQLite engine
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.speed_graph_widget = GraphWidget()  # Create an instance of the GraphWidget for speed
        self.altitude_graph_widget = GraphWidget()  # Create an instance of the GraphWidget for altitude
        self.label_widget = QWidget()  # Create an instance of the label widget

        self.speed_graph_widget.plot_graph([], [])  # Plot empty speed graph initially
        self.altitude_graph_widget.plot_graph([], [])  # Plot empty altitude graph initially
        ...
        altitude_label = QLabel("Altitude:")  # Create a QLabel for altitude
        ...
        altitude_label.setAlignment(Qt.AlignCenter)  # Set alignment of the altitude label to center
        ...
        self.altitude_input = QLineEdit()  # Create a QLineEdit for altitude input
        self.altitude_input.setAlignment(Qt.AlignCenter)  # Set alignment of the altitude input to center
        label_layout.addWidget(altitude_label)  # Add the altitude label to the layout
        label_layout.addWidget(self.altitude_input)  # Add the altitude input to the layout
        ...
        main_layout.addWidget(self.speed_graph_widget)  # Add the speed graph widget to the main layout
        main_layout.addWidget(self.altitude_graph_widget)  # Add the altitude graph widget to the main layout
        ...
        drone_data = DroneData(time_=0, speed=0, altitude=0)  # Create a new DroneData instance
    ...
    def save_data(self):
        ...
        altitude = self.altitude_input.text()  # Get the altitude value from the altitude input

        self.speed_input.clear()  # Clear the speed input
        self.altitude_input.clear()  # Clear the altitude input

        if speed and altitude:
            speed_value = float(speed)  # Convert the speed value to float
            altitude_value = float(altitude)  # Convert the altitude value to float
            ...
            time_ = latest_time[0] + 1 

            drone_data = DroneData(time_=time_, speed=speed_value, altitude=altitude_value)  # Create a new DroneData instance
            self.session.add(drone_data)  # Add the DroneData instance to the session
            self.session.commit()  # Commit the changes to the database
            print("Data saved successfully!")

            self.update_speed_graph()  # Update the speed graph with the new data
            self.update_altitude_graph()  # Update the altitude graph with the new data

    def update_speed_graph(self):
        # Retrieve speed data from SQLAlchemy and extract time and speed values
        ...
        self.speed_graph_widget.clear_graph()  # Clear the speed graph before plotting new data
        self.speed_graph_widget.plot_graph(x_data, y_data)  # Plot the updated speed data

    def update_altitude_graph(self):
        # Retrieve altitude data from SQLAlchemy and extract time and altitude values
        data = self.session.query(DroneData).all()
        x_data = [record.time_ for record in data]  # Extract time values from data
        y_data = [record.altitude for record in data]  # Extract altitude values from data

        self.altitude_graph_widget.clear_graph()  # Clear the altitude graph before plotting new data
        self.altitude_graph_widget.plot_graph(x_data, y_data)  # Plot the updated altitude data
