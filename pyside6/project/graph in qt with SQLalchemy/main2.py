...
# SQLAlchemy setup
engine = create_engine('sqlite:///drone_data4.db.sqlite3')  # Create an SQLite engine
...
class GraphWidget(QWidget):
    def __init__(self, parent=None):
        ...
        #class that represents a whole figure or window in matplotlib,acts as a top-level container for all the elements such as axes, labels
        
        self.canvas = FigureCanvas(self.figure)  # Create a FigureCanvas instance and pass the Figure
        #class that provides the drawing area for a figure,acts as a bridge between the matplotlib figure and the GUI toolkit
    ...
    def plot_graph(self, x_data, y_data):
        ax = self.figure.add_subplot(1,1,1)  # Add a subplot to the Figure
        # is necessary in the plot_graph method to create a subplot within the Figure object.
